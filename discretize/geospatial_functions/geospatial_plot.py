...
def plot_hru_level0_area(level, sub_vector,stream_vector,wgs_crs,
    ...
    inhru_num = insub_num
    title_update = title+' at complexity level '+str(level)+'\n'+' (#GRUs='+str(insub_num)+'. #HRUs='+str(inhru_num)+')'
    ...
    legend_elements = [Line2D([0], [0], color='b', lw=1.5, label='Stream'),
                       Patch(facecolor='white', edgecolor='k', label='GRU(HRU)')]
...
def plot_hru_level1_area(level, sub_vector,hru_vector,stream_vector,wgs_crs,cmap_str,
                         figsize,title,leg_loc,leg_bbox_to_anchor,leg_ncol,ofile):
...
#     hruColors = {'1': 'wheat', '2': 'goldenrod'}
    ...
    plt.show()  
    return
def plot_hru_level2_area(level, sub_vector,hru_vector,stream_vector,wgs_crs,cmap_str,
                         figsize,title,leg_loc,leg_bbox_to_anchor,leg_ncol,ofile):
    # 1. identify HRU classes per subbasin
    inhru_gpd = gpd.read_file(hru_vector)
    inhru_gpd_prj = inhru_gpd.to_crs(wgs_crs)
...
def plot_hru_level3_area(level, sub_vector,hru_vector,stream_vector,wgs_crs,cmap_str,
                         figsize,title,leg_loc,leg_bbox_to_anchor,leg_ncol,ofile):
